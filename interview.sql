...
ALTER TABLE posts
ADD FOREIGN KEY (user_id) REFERENCES clients(id); -- возможно надо было ссылаться на таблицу users, но там максимальное id 6 =)
...
FROM posts, clients 
WHERE posts.user_id = clients.id AND login = "Mikle";